...
static int __init CharDevice_init(void)
{
	...
	result=register_chrdev_region(Mydev,1,"MyIOCTlDevice");//register device region
	if(result<0)
	{
...
static long NAME_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	switch(cmd){
		...
		case GETSTATS:
	             copy_to_user(&status,(struct stats *)arg, sizeof(struct stats));
	             printk("sending status to the user ioctl\n");
	             break;	
...
MODULE_DESCRIPTION("IOCTL functionality");
